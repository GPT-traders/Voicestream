...
#         """
#         print("11111",self.chunk_length_seconds * self.client.sampling_rate * self.client.samples_width)
#         print("2222",len(self.client.buffer))
...
class SilenceAtEndOfChunk(BufferingStrategyInterface):
    def __init__(self, client, **kwargs):
        """
        Initialize the buffering strategy with client specific configurations.
        Args:
            client (Client): The client instance to manage audio processing for.
            **kwargs: Configuration for chunk processing.
        """
        self.client = client
        self.chunk_length_seconds = float(kwargs.get('chunk_length_seconds', os.getenv('BUFFERING_CHUNK_LENGTH_SECONDS', 5)))
        self.chunk_offset_seconds = float(kwargs.get('chunk_offset_seconds', os.getenv('BUFFERING_CHUNK_OFFSET_SECONDS', 0.1)))
        self.error_if_not_realtime = kwargs.get('error_if_not_realtime', os.getenv('ERROR_IF_NOT_REALTIME', False))
        self.processing_flag = False

    def process_audio(self, websocket, vad_pipeline, asr_pipeline):
        """
        Check if the buffer size exceeds the chunk length and schedule processing.
        """
        chunk_length_in_bytes = self.chunk_length_seconds * self.client.sampling_rate * (self.client.samples_width / 8)
        
        if len(self.client.buffer) >= chunk_length_in_bytes:

            print("in the loop")
            if self.processing_flag:
                raise Exception("Error: Concurrent processing detected.")
            
            # Move buffer to scratch for processing and clear main buffer
            self.client.scratch_buffer += self.client.buffer
            self.client.buffer.clear()
            self.processing_flag = True

            print("send for transcription?")
    ...
    async def process_audio_async(self, websocket, vad_pipeline, asr_pipeline):
        ...
        try:
            ...
            print(f"dfdff :: {vad_results}")

            transcription = await asr_pipeline.transcribe(self.client)


            last_segment_should_end_before = ((len(self.client.scratch_buffer) / (self.client.sampling_rate * self.client.samples_width/8)) - self.chunk_offset_seconds)
            if vad_results[-1]['end'] < last_segment_should_end_before:
                transcription = await asr_pipeline.transcribe(self.client)

                print(f"1212121 :: {transcription}")

                if transcription['text'] != '':
                    end = time.time()
                    transcription['processing_time'] = end - start
                    json_transcription=json.dumps(transcription)
                    await websocket.send_json(json_transcription)
